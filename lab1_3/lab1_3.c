...
static void process_image(struct Image img)
{
	for (int y = 0; y < img.height; y++)
		for (int x = 0; x < img.width; x++)
		{
			...
			add_sat(img, x + 1, y + 0, err * 7 / 48.0);
			add_sat(img, x + 2, y + 0, err * 5 / 48.0);

			add_sat(img, x - 2, y + 1, err * 3 / 48.0);
			add_sat(img, x - 1, y + 1, err * 5 / 48.0);
			add_sat(img, x + 0, y + 1, err * 7 / 48.0);
			add_sat(img, x + 1, y + 1, err * 5 / 48.0);
			add_sat(img, x + 2, y + 1, err * 3 / 48.0);

			add_sat(img, x - 2, y + 2, err * 1 / 48.0);
			add_sat(img, x - 1, y + 2, err * 3 / 48.0);
			add_sat(img, x + 0, y + 2, err * 5 / 48.0);
			add_sat(img, x + 1, y + 2, err * 3 / 48.0);
			add_sat(img, x + 2, y + 2, err * 1 / 48.0);
		}
}
